...
#The user can set an initial number of bacteria (blue dots), phages (green dots), and antibiotic molecules (red dots) to interact in a physical space.
...
The extent to which the bacterial population adapts to the presence of phages and antibiotics is measured by tracking the mean antibiotic resistance value over time.')
    ...
    for i in range(NUM_ANTIBIOTICS):
        all_antibiotics.append(Agent([np.random.uniform(0,BOUNDS[0]), np.random.uniform(0,BOUNDS[1])]))
